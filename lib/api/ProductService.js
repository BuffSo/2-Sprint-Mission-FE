...
export async function createProduct(productData) {
  ...
  if (productData instanceof FormData) {
    ...
    if (!name?.trim() || name.length > 20) {
      throw new Error('상품명은 필수이며, 20자 이내로 입력해 주세요.');
    }
    if (!price) {
      throw new Error('가격은 필수 항목입니다.');
    }
  } else {
    if (!productData.name?.trim() || productData.name.length > 20) {
      throw new Error('상품명은 필수이며, 20자 이내로 입력해 주세요.');
    }
...
export const deleteProductComment = async (commentId) => {
  try {
    const res = await instance.delete(`products/comments/${commentId}`);
    return res; 
  } catch (error) {
...
export const updateProductComment = async (commentId, data) => {
  try {
    const res = await instance.patch(`products/comments/${commentId}`, data);
    return res;
  } catch (error) {
