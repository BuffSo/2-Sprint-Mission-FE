...
// 게시글 댓글 목록 조회
export const getArticleComments = async (articleId, limit, cursor = null) => {
  try {
    const params = { limit };
    if (cursor) params.cursor = cursor;

    const res = await instance.get(`/articles/${articleId}/comments`, { params });
    //buff
    //console.log('res', res);
    return res;
  } catch (error) {
...
export async function addArticleFavorite(articleId) {
  try {
    const res = await instance.post(`/articles/${articleId}/favorite`);
    return res;
  } catch (error) {
    // 이미 찜한 경우의 에러 메시지 처리
    if (error.response?.data?.message === '이미 찜한 게시글입니다.') {
      console.warn('이미 찜한 게시글입니다.');
      return { error: '이미 찜한 게시글입니다.' };  // 에러를 throw하지 않고 반환
    }
    console.error('게시글 좋아요 추가에 실패했습니다:', error);
    throw error;
  }
}
...
export async function removeArticleFavorite(articleId) {
  try {
    const res = await instance.delete(`/articles/${articleId}/favorite`);
    return res;
  } catch (error) {
    console.error('게시글 좋아요 취소에 실패했습니다:', error);
    throw error;
  }
}